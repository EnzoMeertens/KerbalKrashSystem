...
public static class MeshHelper
{
    ...
    public static void Subdivide4(Mesh mesh, Vector3 hitpoint, float distance)
    {
        ...
        int[] triangles = mesh.triangles; //Only use the triangles closest to the dent...?
    ...
    /// </param>
    public static void Subdivide(Mesh mesh, int level, Vector3 hitpoint, float distance)
    {
        if (level < 2)
            return;
        while (level > 1)
        {
            ...
            while (level % 2 == 0)
            {
                Subdivide4(mesh, hitpoint, distance);
                level /= 2;
            }
